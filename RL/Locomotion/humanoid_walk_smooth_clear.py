...
RUN_DIR = Path("runs/simple_humanoid_smooth_clear_walk")
SEED = 42
...
class SmoothActionReward(gym.Wrapper):
...
class FootClearanceReward(gym.Wrapper):
    def __init__(self, env, weight=0.2):
        super().__init__(env)
        self.weight = weight

        model = self.unwrapped.model
        self.right_foot_id = model.body("right_foot").id
        self.left_foot_id = model.body("left_foot").id

    def step(self, action):
        obs, reward, terminated, truncated, info = self.env.step(action)

        clearance_reward = self._clearance_reward()
        reward = reward + self.weight * clearance_reward

        info["custom/clearance_reward"] = clearance_reward
        return obs, reward, terminated, truncated, info
...
def make_env(render_mode: str | None = None):
    ...
    env = SmoothActionReward(env, weight=0.08)
    env = FootClearanceReward(env, weight=0.1)
    return env

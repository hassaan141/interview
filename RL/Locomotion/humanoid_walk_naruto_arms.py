...
RUN_DIR = Path("runs/simple_humanoid_naruto_arms_walk")
SEED = 42
...
class ArmsBackReward(gym.Wrapper):
    def __init__(self, env, weight=0.3, target_back_distance=0.45, target_stretch=0.45):
        super().__init__(env)
        self.weight = weight
        self.target_back_distance = target_back_distance
    ...
    def step(self, action):
        ...
        arms_back_reward = self._arms_back_reward()
        reward += self.weight * arms_back_reward

        info["custom/arms_back_reward"] = arms_back_reward
        return obs, reward, terminated, truncated, info
    ...
    def _arms_back_reward(self):
        ...
        right_back_score = min(right_back / self.target_back_distance, 1.0)
        left_back_score = min(left_back / self.target_back_distance, 1.0)

        right_stretch = np.linalg.norm(right_hand - right_shoulder)
        left_stretch = np.linalg.norm(left_hand - left_shoulder)

        right_stretch_score = min(float(right_stretch / self.target_stretch), 1.0)
        left_stretch_score = min(float(left_stretch / self.target_stretch), 1.0)

        right_score = right_back_score * right_stretch_score
        left_score = left_back_score * left_stretch_score
        return min(right_score, left_score)


class ForwardLeanReward(gym.Wrapper):
    def __init__(self, env, weight=0.15, target_lean=0.25):
        super().__init__(env)
        self.weight = weight
        self.target_lean = target_lean

        model = self.unwrapped.model
        self.head_body_id = model.body("torso").id
        self.pelvis_body_id = model.body("pelvis").id

    def step(self, action):
        obs, reward, terminated, truncated, info = self.env.step(action)

        lean_reward = self._forward_lean_reward()
        reward += self.weight * lean_reward

        info["custom/forward_lean_reward"] = lean_reward
        return obs, reward, terminated, truncated, info

    def _forward_lean_reward(self):
        data = self.unwrapped.data
        head_x = data.xpos[self.head_body_id][0]
        pelvis_x = data.xpos[self.pelvis_body_id][0]
        forward_lean = max(float(head_x - pelvis_x), 0.0)
        return np.exp(-20.0 * (forward_lean - self.target_lean) ** 2)
...
def make_env(render_mode: str | None = None):
    ...
    env = ArmsBackReward(env, weight=0.3, target_back_distance=0.45, target_stretch=0.45)
    env = ForwardLeanReward(env, weight=0.15, target_lean=0.25)
    return env
